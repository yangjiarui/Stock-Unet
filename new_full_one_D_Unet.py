...
def test(struddle,df,labels):
 ...
 #   cur=sigmoid(df.values)
    ...
    df=pd.DataFrame(cur.reshape(-1),columns=df.columns)   #cur.reshape(-1)
    ...
    model.fit(train_df.reshape(-1,len(df),1),np.array(labels).reshape(-1,len(labels),1),
              verbose=1,batch_size=1,epochs=60)#validation_split=0.2)#,callbacks=[model_checkpoint])  # acc:  around better 70%
   ...
   # model.save(str(struddle)+'model.h5')
    ...
    open(str(struddle)+'_model_architecture.json', 'w').write(json_string)
    model.save_weights(str(struddle)+'_model_weights.h5')
    # 加载模型数据和weights
...
#final=[
file_list=os.listdir('/home/jerry/inteligient/MASTER_project/factors/avalible_REITs')[12:13]
...
def Main(start,end,interval):
    ...
    for i in range(start,end,interval):
        ...
        struddle=i
        for a,b,c,d,oringn_df in test(struddle,df,labels):
            name.append(a)
...
def jugment(cur_index,full_df):

    if full_df.iloc[cur_index - 1, :].values >= full_df.iloc[cur_index, :].values:
        return 'bigger'
    else:
        return 'lowwer'
       ...
       # print(cur_index)
        if cur_index == 0:
           ...
           # if defined_df['next_trading_day'][cur_index]==jugment(cur_index,full_df):
            if defined_df['next_trading_day'][seq] == jugment(cur_index, full_df):
                j+=1
                good.append(seq)
...
NUM_col=17
resualt_file,oring_df=Main(start=1,end=5,interval=1)
good,score=evaluate_mode(resualt_file)


pre=pd.read_csv('/home/jerry/inteligient/MASTER_project/factors/avalible_REITs/'+file_list[0],header=None)
pre=MinMaxScaler().fit_transform(pre.iloc[:,1].values.reshape(-1,1))[:4096]
#from keras.models import load_model

look=[]
if len(good)>=1 and len(good)%2==1:
    for g in good:
        new_model = model_from_json(open(str(g)+'_model_architecture.json').read())
        new_model.load_weights((str(g))+'_model_weights.h5')
