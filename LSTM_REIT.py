...
for name in list_names[0:]:
    nod=pd.DataFrame()
    print '\n'
    print 10*'%'+' working on : '+str(name)+' '+10*'$'
    print '\n'
    df=pd.read_csv('/media/jerry/JERRY/MASTER_project(0)/a_all_data/'+name)
    df.index=pd.DatetimeIndex(df.iloc[:,2])
    df=standarized_data_Serires(Serise=df.iloc[:,3],foward_steps=3)
    dff=pd.DataFrame()
    dff[name.split('.')[0]] = df
    dff['ADD_ed']=pd.Series(index=df.index)
    ...
    look_back = 4
    trainX, trainY = create_dataset(train, look_back, nub_of_the_col_index=0)
    ...
    print (model.summary())
    model.fit(trainX, trainY, epochs=10, batch_size=4, verbose=2, validation_split=0.2)
