...
from keras.layers import Dense,Dropout,Embedding,Conv1D,GlobalAveragePooling1D,MaxPooling1D,AveragePooling1D,Reshape
x=[]
...
def get_1d_unet():
    ...
    conv1=Conv1D(8,kernel_size=5,strides=1,activation='relu',padding='same',kernel_initializer='glorot_uniform')(inputs)
    conv2=Conv1D(16,kernel_size=5,strides=1, activation='relu',padding='same')(conv1)
    conv2=Dropout(0.2)(conv2)
    #conv2=Conv1D(8,4,activation='relu',padding='same')(conv2)
    print('conv2',conv2)
    polling1d=AveragePooling1D(pool_size=2)(conv2)
    #(2048,128)
    conv3=Conv1D(16, kernel_size=5,strides=1, activation='relu',padding='same')(polling1d)
    conv3 = Dropout(0.2)(conv3)
    conv3=Conv1D(32, kernel_size=5,strides=1, activation='relu',padding='same')(conv3)
    polling2d=AveragePooling1D(pool_size=2)(conv3)
    #(1024,256)
    ...
    conv4=Dropout(0.2)(conv4)
    conv4=Conv1D(64,kernel_size=5,strides=1,activation='relu',padding='same')(conv4)
    #conv4 = Conv1D(32, 4, activation='relu', padding='same')(conv4)
    polling3d=AveragePooling1D(pool_size=2)(conv4)
 ...
 #   print(conv5)
    ...
    conv5=Conv1D(128,kernel_size=5,strides=1,activation='relu',padding='same')(conv5)
    #conv5=Conv1D(64,4,activation='relu',padding='same')(conv5)
    polling4d=AveragePooling1D(pool_size=2)(conv5)
    ...
    polling5d = AveragePooling1D(pool_size=2)(conv6)
    #(128,2048)
    ...
    polling6d = AveragePooling1D(pool_size=2)(conv7)
    #(64,4096)
    ...
    polling7d = AveragePooling1D(pool_size=2)(conv8)
    #(32,4096*2)
    conv9 = Conv1D(1024, kernel_size=5,strides=1, activation='relu', padding='same')(polling7d)
    conv9 = Dropout(0.4)(conv9)
    conv9 = Conv1D(2048, kernel_size=5,strides=1, activation='relu', padding='same')(conv9)
   ...
   #change the correspoding_upsampling_layers
    ...
    upconv1=Conv1D(1024,kernel_size=5,strides=1,activation='relu',padding='same')(upconv1)
    #(64,)
    print('cov6',conv6)
    print('conv2',conv2)
    print('conv3',conv3)
...
#    print('up2',up2)
    ...
    upconv4 = Conv1D(128 , kernel_size=5,strides=1, activation='relu', padding='same')(up4)
    upconv4 = Dropout(0.2)(upconv4)
    upconv4 = Conv1D(128 , kernel_size=5,strides=1, activation='relu', padding='same')(upconv4)
    #(1024,2048)
    ...
    #
    convend=Conv1D(8,kernel_size=5,strides=1,activation='relu',padding='same')(upconv7)
    convend=Dropout(0.4)(convend)
    convend=Conv1D(3,kernel_size=5,strides=1,activation='relu',padding='same')(convend)
    convend=Dropout(0.6)(convend)
    convend=Conv1D(1,kernel_size=5,strides=1,activation='relu',padding='same')(convend)
    #convend = Flatten()(convend)
    #print(convend)
    #convend = Reshape((-1, 3))(convend)
    #convend = Dropout(0.4)(convend)
    #convend = Dense(100, activation='relu')(convend)
...
def test(df,label):
   # global df
    ...
    ds=MinMaxScaler().fit_transform(df.reshape(-1,1))
    ds=StandardScaler().fit_transform(df.reshape(-1,1))
    pd.DataFrame(ds).to_csv('ds.csv')
    model.fit(ds.reshape(1,len(ds),1),np.array(label).reshape(1,len(label),-1),batch_size=1,epochs=40)#,callbacks=[model_checkpoint])  # acc:  around better 70%
    loss,acc=model.evaluate(ds.reshape(1,len(ds),1),np.array(label).reshape(1,len(label),-1))


    #loss=model.predict(scale(df).reshape(1,len(df),1))-np.array(label).reshape(1,len(label),1)
    w=pd.DataFrame()
    w['label']=pd.Series(label)
    w['predicted']=pd.Series(model.predict(ds.reshape(1,len(ds),1)).reshape(-1))
#    w['oringin']=pd.Series(df.values)
    ...
    if w.iloc[0,1]==q.iloc[:,1]['min']:
        ...
        return df.index[0],'next_trading_day:bigger',acc
    else:
        #print('tomorrow is lowwer than today')
        return df.index[0],'next_trading_day:lowwer',acc
...
#def MainRange(start,stop):
    for i in range(3,7,1):
        ...
        ds = pd.read_excel('/home/jerry/PycharmProjects/untitled/venv/MASTER_project/stocks/aapl.xlsx')
        ds = ds.fillna(method='bfill')
        ds = ds.fillna(method='ffill')
        ds = ds[i:4096 + i]  # cut the oringin data
        df = ds.iloc[0:, [1, 2]]
