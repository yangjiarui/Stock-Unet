...
from keras.layers import AveragePooling1D,MaxPooling1D,Flatten,Dense,Reshape
...
#    print('up2',up2)
    ...
    convend=Conv1D(5,kernel_size=5,strides=1,activation='relu',padding='same')(convend)
    ...
    #convend = Dense(1, activation='softmax')(convend)
...
def test(struddle,df,labels):
    ...
    #train_df=(df-mea) / std
    train_df=MinMaxScaler().fit_transform(df)   #changed it
    ...
    model.fit(train_df.reshape(-1,len(df),1),np.array(labels).reshape(-1,len(labels),1),
              verbose=1,batch_size=1,callbacks=cbks,epochs=25)#validation_split=0.2)#,callbacks=[model_checkpoint])  # acc:  around better 70%
    ...
    w={}
    w['label']=labels
    w['predicted']=pd.DataFrame(model.predict(train_df.reshape(-1,len(df),1)).reshape(4096,-1))
    w['oringin']=df
...
#final=[
file_list=os.listdir('stocks')[11:12]
def make_df_from_files(file_list):
    ...
    for i in file_list:
       ...
       # data=data[:5000]
        ...
        index[i]=data.iloc[:,1]
        close=data.iloc[:, 2]
        w[i]=np.log(close/close.shift(1))    #has changed
...
def Main():
    ...
    for i in range(1,5,1):
        ...
        struddle=i
        for a,b,c,d in test(struddle,df,labels):
            name.append(a)
